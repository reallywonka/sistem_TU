...
<div class="mb-6">
    ...
    <p class="mt-0.5 text-sm text-gray-500">Edit data pengguna <span class="font-medium">{{ $user->nama_lengkap }}</span>.</p>
</div>

<form method="POST" action="{{ route('users.update', $user->id_user) }}" novalidate>
    @csrf @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
                ...
                <select id="role" name="role" required
                    ...
                    <option value="admin_tu" {{ old('role', $user->role) === 'admin_tu' ? 'selected' : '' }}>Admin TU</option>
    ...
    <div class="flex items-center justify-end gap-3">
        ...
        <button type="submit" id="btn-update-user" class="rounded-lg bg-blue-600 px-5 py-2 text-sm font-semibold text-white hover:bg-blue-700 transition-colors shadow-sm">Simpan Perubahan</button>
    </div>
</form>
@endsection
